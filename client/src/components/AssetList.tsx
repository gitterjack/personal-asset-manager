...
interface TireConfig {
    ...
    spec: string;
    dot: string;
    dotWeekYear: string; // e.g., '(2025年第19周)'
    status: string;
...
const TireMaintenanceSection: React.FC<{ assetId: number }> = ({ assetId }) => {
    ...
    const [tires, setTires] = useState<TireConfig[]>([
        ...
        { id: '2', position: '右前轮', positionCode: 'Front-R', brand: '马牌 MC6', spec: '98V', dot: '1625', dotWeekYear: '2025年第16周', status: '准新胎，由原后轮调入', statusColor: 'text-green-600' },
        { id: '3', position: '左后轮', positionCode: 'Rear-L', brand: '马牌 MC7', spec: '102Y XL', dot: '3125', dotWeekYear: '2025年第31周', status: '加强型，负责抓地防甩尾', statusColor: 'text-blue-600' },
        { id: '4', position: '右后轮', positionCode: 'Rear-R', brand: '马牌 MC7', spec: '102Y XL', dot: '3325', dotWeekYear: '2025年第33周', status: '全新胎，刚刚更换', statusColor: 'text-blue-600' },
    ]);
                ...
                <div>
                    {isEditing ? (
                        <div className="flex gap-2">
                            <button onClick={handleCancel} className="p-1 text-slate-400 hover:text-slate-600 rounded hover:bg-slate-200 transition-colors" title="取消">
                                <X size={16} />
                            </button>
                            <button onClick={handleSave} className="p-1 text-green-600 hover:text-green-700 rounded hover:bg-green-50 transition-colors" title="保存">
                                <Save size={16} />
                            </button>
                        </div>
                    ) : (
                        <button onClick={handleEdit} className="p-1 text-slate-400 hover:text-blue-600 rounded hover:bg-slate-200 transition-colors" title="编辑配置">
                            <Edit2 size={16} />
                        </button>
                    )}
                </div>
            ...
            <div className="overflow-x-auto">
                <table className="w-full text-left text-xs text-slate-600">
                    <thead className="bg-slate-50 text-slate-500 font-medium border-b border-slate-100">
                        <tr>
                            <th className="px-4 py-2 font-medium w-32">轮胎位置</th>
                            <th className="px-4 py-2 font-medium">品牌花纹</th>
                            <th className="px-4 py-2 font-medium">规格参数</th>
                            <th className="px-4 py-2 font-medium">DOT 周期</th>
